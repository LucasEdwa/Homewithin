...
const CRISIS_KEYWORDS = [
  ...
  'self harm', 'selfharm', 'overdose', 'cutting myself', 'no reason to live',
  'can\'t go on', 'cannot go on', 'end it all',
];
...
export async function sendMessage(
  ...
  const { data, error } = await supabase
    .from('messages')
    .insert({ match_id: matchId, sender_id: user.id, body, expires_at: expiresAt })
    .select()
    .single();
...
export async function getMessages(matchId: string): Promise<Message[]> {
  ...
  if (error) { console.error('Get messages failed:', error.message); return []; }
